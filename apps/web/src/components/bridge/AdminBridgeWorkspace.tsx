...
export function AdminBridgeWorkspace({ locale }: { locale: Locale }) {
  ...
  async function refund(item: Refund) { const attempt=item.order_item.order.payment_attempts[0]; if (!attempt) return; const idempotencyKey = refundKeys.current[attempt.id] ??= crypto.randomUUID(); setBusy(item.id); try { await api.post(`/payments/admin/${attempt.id}/refund`, { reason: "Approved Bridge fulfillment refund request" }, { headers: { "Idempotency-Key": idempotencyKey } }); delete refundKeys.current[attempt.id]; await load(); } catch (requestError) { setError(safeApiError(requestError, ERROR_COPY[locale])); } finally { setBusy(""); } }

  return <section className={styles.workspace} aria-labelledby="admin-bridge-title">
    <header className={styles.dashboardHeader}><div><p className={styles.brand}>{c.brand}</p><h1 id="admin-bridge-title">{c.title}</h1><p>{c.intro}</p></div></header>
    ...
    <section className={styles.section}><div className={styles.sectionHead}><h2>{c.refunds}</h2></div>{refunds.length ? <div className={styles.grid}>{refunds.map((item) => <article className={styles.card} key={item.id}><h3>#{item.order_item.order.id}</h3><p>{item.order_item.order.seller.shop_name}</p><p>{formatCurrencyAmount(item.order_item.order.total_amount, item.order_item.order.currency, locale)} {currencyLabel(item.order_item.order.currency)}</p><button className={styles.button} disabled={busy===item.id || !item.order_item.order.payment_attempts.length} onClick={() => void refund(item)}>{c.refund}</button></article>)}</div> : <p className={styles.empty}>{c.no}</p>}</section>
