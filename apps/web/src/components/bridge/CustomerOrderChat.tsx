...
export function CustomerOrderChat({ orderId, available, c }: { orderId: string; available: boolean; c: OrderCopy }) {
  ...
  async function open() {
    ...
    try {
      ...
      if (settings.enabled) await openGoghdiOrderTicket(orderId);
    } catch (cause) {
  ...
  return <div className={s.sellerChat}>
    <p id="order-chat-hint" role="status">{hint}</p>
    <button type="button" className={`${s.primary} ${s.chatButton}`} aria-describedby="order-chat-hint" aria-busy={busy} disabled={busy || config === "loading" || config === "disabled"} onClick={() => void open()}>
