...
export function BridgeProductForm({ locale, initialGrantId }: { locale: Locale; initialGrantId?: string }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...
      offers: variants.map((variant) => ({ ...(kind === "variable" ? { variantKey: `variant-${variant.id}` } : {}), price: tomanToIrr(variant.price.trim()), currency: "IRR", status: "active" }))
    };
