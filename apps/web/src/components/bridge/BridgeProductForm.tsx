...
export function BridgeProductForm({ locale, initialGrantId }: { locale: Locale; initialGrantId?: string }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...
      offers: variants.map((variant) => ({ ...(kind === "variable" ? { variantKey: `variant-${variant.id}` } : {}), price: variant.price.trim(), currency, status: "active" }))
    };
    ...
    <main className={styles.main}>
      ...
      {selected ? <form className={styles.form} onSubmit={submit} aria-busy={busy}>
        ...
        <section className={styles.section}><div className={styles.sectionHead}><h2>{kind === "variable" ? c.variant : c.price}</h2>{kind === "variable" ? <button className={styles.buttonQuiet} type="button" onClick={() => setVariants((items) => [...items, { id: Date.now(), name: "", value: "", price: "" }])}>{c.add}</button> : null}</div>
          <label className={styles.field}><span>{locale === "fa" ? "واحد پول" : locale === "ar" ? "العملة" : "Currency"}</span><select value={currency} onChange={(e) => setCurrency(e.target.value as "TOMAN" | "USD")}><option value="TOMAN">تومان</option><option value="USD">USD</option></select><small>&nbsp;</small></label>
          ...
          <div className={styles.grid}>{variants.map((variant) => <article className={styles.card} key={variant.id}>{kind === "variable" ? <><label className={styles.field}><span>{c.variant}</span><input value={variant.name} onChange={(e) => updateVariant(variant.id, "name", e.target.value)} maxLength={200}/><small>&nbsp;</small></label><label className={styles.field}><span>{c.value}</span><input required value={variant.value} onChange={(e) => updateVariant(variant.id, "value", e.target.value)} maxLength={100}/><small>&nbsp;</small></label></> : null}<label className={styles.field}><span>{c.price}</span><input required inputMode="decimal" pattern="(?:0|[1-9][0-9]{0,15})(?:\.[0-9]{1,4})?" value={variant.price} onChange={(e) => updateVariant(variant.id, "price", e.target.value)}/><small>{currency === "USD" ? "USD" : "تومان"}</small></label>{kind === "variable" && variants.length > 1 ? <button className={styles.buttonQuiet} type="button" onClick={() => setVariants((items) => items.filter((item) => item.id !== variant.id))}>{c.remove}</button> : null}</article>)}</div>
        </section>
