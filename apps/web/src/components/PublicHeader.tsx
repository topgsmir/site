...
export function PublicHeader({ locale, accountHref, current }: { locale: Locale; accountHref?: string | null; current?: "shop" | "journal" | "cart" }) {
  const c = copy[locale];
  ...
  return <header className={styles.header}>
    ...
    <div className={styles.actions}><CartLink locale={locale} className={styles.cart} current={current === "cart"} />
      <div className={styles.languages} aria-label="Language">{(["fa", "en", "ar"] as const).map((language) => <a key={language} href={`/${language}${currentPath}` as Route} hrefLang={language} aria-current={locale === language ? "page" : undefined}>{language.toUpperCase()}</a>)}</div>
      <details className={styles.languageMenu} suppressHydrationWarning><summary aria-label="Language">{locale.toUpperCase()}</summary><nav aria-label="Language">{(["fa", "en", "ar"] as const).map((language) => <a key={language} href={`/${language}${currentPath}` as Route} hrefLang={language} aria-current={locale === language ? "page" : undefined}>{language === "fa" ? "فارسی" : language === "ar" ? "العربية" : "English"}</a>)}</nav></details>
