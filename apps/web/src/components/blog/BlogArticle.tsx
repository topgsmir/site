...
import type { PublicBlogPost, PublicProductSummary, RelatedProductSummary } from "@topgsm/shared-types";
...
export function BlogArticle({ locale, post, storeProducts = [] }: { locale: Locale; post: PublicBlogPost; storeProducts?: PublicProductSummary[] }) {
  ...
  const products: ArticleProduct[] = (post.relatedProducts.length ? post.relatedProducts : storeProducts).slice(0, 3);
  return (
      ...
      <main id="article-content" className={styles.main}>
        ...
        <div className={styles.layout}>
          <article className={styles.article}>
            ...
            <a className={styles.mobileShopLink} href="#article-shop-title"><DesignIcon name="bag" /><span>{post.relatedProducts.length ? copy.related : copy.shop}</span><DesignIcon name="arrow" /></a>
          ...
          <aside className={styles.sidebar} aria-label={copy.promotion}>
            <section className={styles.shop} aria-labelledby="article-shop-title">
              <div className={styles.shopIntro}>
                ...
                <h2 id="article-shop-title">{post.relatedProducts.length ? copy.related : copy.products}</h2>
                <p>{copy.intro}</p>
                <Link className={styles.shopCta} href={`/${locale}/products` as Route}>{copy.browse}<DesignIcon name="arrow" /></Link>
              </div>
