...
const copy = {
  en: { title: "Product comments", locked: "Answer unanswered comments or flag spam to return to your workspace.", normal: "Reply to customer comments about products you sell.", loading: "Loading comments…", empty: "No comments need your attention.", reply: "Reply", replying: "Sending…", flag: "Flag as spam", answer: "Your answer", more: "Load more", error: "Comments could not be loaded. Try again.", actionError: "The action failed. Refresh and try again.", back: "Back to dashboard", outstanding: "unanswered", refresh: "Refresh" },
  fa: { title: "دیدگاه‌های محصولات", locked: "برای بازگشت به پنل، به دیدگاه‌های بی‌پاسخ جواب دهید یا موارد هرزنامه را گزارش کنید.", normal: "به دیدگاه‌های مشتریان درباره محصولاتتان پاسخ دهید.", loading: "در حال دریافت دیدگاه‌ها…", empty: "دیدگاهی در انتظار پاسخ نیست.", reply: "ارسال پاسخ", replying: "در حال ارسال…", flag: "گزارش هرزنامه", answer: "پاسخ شما", more: "نمایش بیشتر", error: "دیدگاه‌ها دریافت نشدند. دوباره تلاش کنید.", actionError: "انجام این کار ممکن نبود. صفحه را تازه کنید.", back: "بازگشت به پنل", outstanding: "بی‌پاسخ", refresh: "تازه‌سازی" },
  ar: { title: "تعليقات المنتجات", locked: "أجب عن التعليقات أو أبلغ عن الرسائل المزعجة للعودة إلى لوحة البائع.", normal: "أجب عن تعليقات العملاء على المنتجات التي تبيعها.", loading: "جار تحميل التعليقات…", empty: "لا توجد تعليقات تنتظر ردك.", reply: "إرسال الرد", replying: "جار الإرسال…", flag: "الإبلاغ كرسالة مزعجة", answer: "ردك", more: "عرض المزيد", error: "تعذر تحميل التعليقات. حاول مجددًا.", actionError: "تعذر إكمال الإجراء. حدّث الصفحة.", back: "العودة إلى اللوحة", outstanding: "بلا رد", refresh: "تحديث" }
} as const;
...
export function SellerCommentsWorkspace({ locale, locked = false }: { locale: Locale; locked?: boolean }) {
  ...
  const load = useCallback(async (next?: string) => {
    ...
    try {
      const response = await api.get<CommentPage<SellerComment>>("/comments/seller", { params: { limit: 20, ...(next ? { cursor: next } : {}) } });
  ...
  return <main className={styles.section} dir={locale === "en" ? "ltr" : "rtl"}>
    ...
    <div className={styles.list}>{items.map((item) => <article className={styles.card} key={item.id}>
      <div className={styles.meta}><strong>{item.productTitle}</strong><span>· {item.authorName}</span><time dateTime={item.createdAt}>{new Date(item.createdAt).toLocaleDateString(locale)}</time></div>
      <p>{item.body}</p>
