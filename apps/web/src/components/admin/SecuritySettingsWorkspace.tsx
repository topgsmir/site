...
const groups = [
  { name: ["Access", "دسترسی", "الوصول"], actions: ["login", "register", "otp", "staff_setup", "auth_configuration", "captcha_challenge"] },
  { name: ["Commerce", "تجارت", "التجارة"], actions: ["checkout_quote", "order", "shipping", "shipping_configuration", "payout", "payment", "payment_callback", "payment_refund", "payment_configuration"] },
  { name: ["Content", "محتوا", "المحتوى"], actions: ["media", "comment_submit_guest", "comment_submit", "comment_reply", "comment_admin", "product_bulk_undo"] },
  { name: ["Platform", "پلتفرم", "المنصة"], actions: ["sms_configuration", "bridge", "signed_ticket", "ai_profile", "ai_profile_test", "ai_run", "analytics"] }
] as const;
const actionNames: Record<string, [string, string, string]> = {
  ...
  staff_setup: ["Staff setup", "راه‌اندازی کارکنان", "إعداد الموظفين"], auth_configuration: ["Authentication settings", "تنظیمات احراز هویت", "إعدادات المصادقة"], captcha_challenge: ["Browser challenge requests", "درخواست بررسی مرورگر", "طلبات فحص المتصفح"],
  checkout_quote: ["Checkout quotes", "استعلام تسویه‌حساب", "تسعير الدفع"], order: ["Orders", "سفارش‌ها", "الطلبات"], shipping: ["Shipping updates", "به‌روزرسانی ارسال", "تحديث الشحن"], shipping_configuration: ["Shipping settings", "تنظیمات ارسال", "إعدادات الشحن"], payout: ["Payouts", "تسویه‌ها", "المدفوعات للبائعين"], payment: ["Payment initiation", "آغاز پرداخت", "بدء الدفع"], payment_callback: ["Payment callbacks", "بازگشت پرداخت", "استدعاءات الدفع"], payment_refund: ["Refunds", "بازپرداخت‌ها", "المبالغ المستردة"], payment_configuration: ["Payment settings", "تنظیمات پرداخت", "إعدادات الدفع"],
  media: ["Media uploads", "بارگذاری رسانه", "رفع الوسائط"], comment_submit_guest: ["Guest comments", "دیدگاه مهمان", "تعليقات الضيوف"], comment_submit: ["Signed-in comments", "دیدگاه کاربران واردشده", "تعليقات المستخدمين"], comment_reply: ["Seller comment replies", "پاسخ فروشنده", "ردود البائعين"], comment_admin: ["Comment moderation", "مدیریت دیدگاه", "إدارة التعليقات"], product_bulk_undo: ["Bulk product undo", "بازگردانی گروهی محصول", "التراجع الجماعي عن المنتجات"],
  sms_configuration: ["SMS settings", "تنظیمات پیامک", "إعدادات الرسائل"], bridge: ["Bridge operations", "عملیات بریج", "عمليات الجسر"], signed_ticket: ["Signed tickets", "بلیت‌های امضاشده", "التذاكر الموقعة"], ai_profile: ["AI profiles", "نمایه‌های هوش مصنوعی", "ملفات الذكاء الاصطناعي"], ai_profile_test: ["AI profile tests", "آزمایش نمایه هوش مصنوعی", "اختبارات ملفات الذكاء الاصطناعي"], ai_run: ["AI runs", "اجرای هوش مصنوعی", "تشغيل الذكاء الاصطناعي"], analytics: ["Analytics reads", "خواندن گزارش‌ها", "قراءة التحليلات"]
};
const actionDescriptions: Record<string, [string, string, string]> = {
  ...
  shipping: ["Registering or syncing an order shipment with Amadast.", "ثبت یا همگام‌سازی ارسال سفارش با آمادست.", "تسجيل شحنة الطلب أو مزامنتها مع أمادست."],
  ...
  comment_submit_guest: ["Guests posting comments on product pages.", "ثبت دیدگاه مهمان در صفحه محصول.", "نشر الضيوف لتعليقات على صفحات المنتجات."],
  comment_submit: ["Signed-in users posting product comments.", "ثبت دیدگاه محصول توسط کاربر واردشده.", "نشر المستخدمين المسجلين لتعليقات المنتجات."],
  comment_reply: ["Sellers replying to or flagging product comments.", "پاسخ فروشنده به دیدگاه محصول یا گزارش آن.", "رد البائعين على تعليقات المنتجات أو الإبلاغ عنها."],
  ...
  signed_ticket: ["Issuing signed product tickets for authenticated users.", "صدور بلیت امضاشده محصول برای کاربر واردشده.", "إصدار تذاكر منتجات موقعة للمستخدمين المسجلين."],
