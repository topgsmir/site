...
const copy = {
  en: {
    ...
    userId: "Amadast user ID", storeId: "Amadast store ID", senderName: "Default sender name (optional)", senderMobile: "Default sender mobile (optional)", productType: "Product type ID", packageType: "Package type ID",
  ...
  fa: {
    ...
    userId: "شناسه کاربر آمادست", storeId: "شناسه فروشگاه آمادست", senderName: "نام پیش‌فرض فرستنده (اختیاری)", senderMobile: "موبایل پیش‌فرض فرستنده (اختیاری)", productType: "شناسه نوع محصول", packageType: "شناسه نوع بسته‌بندی",
  ...
  ar: {
    ...
    userId: "معرّف مستخدم Amadast", storeId: "معرّف متجر Amadast", senderName: "اسم المرسل الافتراضي (اختياري)", senderMobile: "جوال المرسل الافتراضي (اختياري)", productType: "معرّف نوع المنتج", packageType: "معرّف نوع التغليف",
...
type FormState = { userId: string; storeId: string; senderName: string; senderMobile: string; productType: string; packageType: string };
const fromSettings = (settings: AdminShippingSettings): FormState => ({
  userId: settings.userId?.toString() ?? "", storeId: settings.storeId?.toString() ?? "", senderName: settings.senderName ?? "",
  senderMobile: settings.senderMobile ?? "", productType: settings.productType.toString(), packageType: settings.packageType.toString()
});
...
export function ShippingSettingsWorkspace({ locale }: { locale: Locale }) {
  ...
  const [form, setForm] = useState<FormState>({ userId: "", storeId: "", senderName: "", senderMobile: "", productType: "1", packageType: "1" });
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await api.patch<AdminShippingSettings>("/admin/settings/shipping", {
        ...
        senderName: form.senderName.trim() || null, senderMobile: form.senderMobile.trim() || null,
      ...
      <form className={styles.form} onSubmit={submit}>
        <section className={styles.credentials} aria-labelledby="shipping-provider-title">
          ...
          <fieldset><legend>{c.credentials}</legend>{([
            ["userId", c.userId, "number"], ["storeId", c.storeId, "number"], ["senderName", c.senderName, "text"], ["senderMobile", c.senderMobile, "tel"], ["productType", c.productType, "number"], ["packageType", c.packageType, "number"]
          ] as const).map(([key, label, type]) => <label key={key}><span>{label}</span><input type={type} inputMode={type === "number" ? "numeric" : undefined} min={type === "number" ? 1 : undefined} max={type === "number" ? 2_147_483_647 : undefined} maxLength={type === "text" ? 200 : undefined} value={form[key]} disabled={saving} onChange={(event) => change(key, event.target.value)} /></label>)}</fieldset>
        </section>
