...
export function AdminBlogWorkspace({ locale }: { locale: Locale }) {
  ...
  async function create(event: FormEvent) {
    event.preventDefault(); setMessage(c.saving);
    try {
  ...
  return <section className={styles.workspace} aria-labelledby="editorial-title"><header className={styles.header}><h1 id="editorial-title">{c.title}</h1><p>{c.intro}</p></header><div className={styles.content}><SellerBlogPanel locale={locale} editBase="admin/blog" /><aside className={styles.taxonomy}><header><h2>{c.taxonomy}</h2><p>{c.taxonomyHelp}</p></header><div className={styles.kindTabs}><button type="button" aria-pressed={kind === "category"} onClick={() => setKind("category")}>{c.categories}</button><button type="button" aria-pressed={kind === "tag"} onClick={() => setKind("tag")}>{c.tags}</button></div><form className={styles.form} onSubmit={create}>{LOCALES.map((code) => <fieldset key={code}><legend>{code.toUpperCase()}</legend><label>{c.name}<input required maxLength={100} value={draft[code].name} onChange={(event) => setDraft((current) => ({ ...current, [code]: { ...current[code], name: event.target.value } }))} /></label><label>{c.slug}<input required maxLength={120} dir="ltr" pattern="[\p{L}\p{N}]+(?:-[\p{L}\p{N}]+)*" value={draft[code].slug} onChange={(event) => setDraft((current) => ({ ...current, [code]: { ...current[code], slug: event.target.value } }))} /></label></fieldset>)}<button type="submit">{kind === "category" ? c.addCategory : c.addTag}</button></form><p role="status">{message}</p><div className={styles.terms}>{terms.map((term) => <div className={styles.term} key={term.id}><strong>{term.translations.find((translation) => translation.locale === locale)?.name ?? term.translations[0]?.name}</strong><span>{term.translations.length}/3</span></div>)}</div></aside></div></section>;
}
