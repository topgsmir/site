...
export default async function ProductsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return <div className="product-document"><header className="product-nav"><Link href={`/${locale}` as Route}>TOP GSM</Link><Link href={`/${locale}/blog` as Route}>Journal</Link></header><main><header className="product-heading"><p>CATALOG / {String(products.length).padStart(2, "0")}</p><h1>{copy.title}</h1><p>{copy.intro}</p></header><section className="product-offers" aria-labelledby="catalog-title"><header><span>01</span><h2 id="catalog-title">{copy.title}</h2></header>{products.length ? <div>{products.map((product) => <article key={product.id}><p>{product.category ?? product.type}</p><h3><Link href={`/${locale}/products/${product.slug}` as Route}>{product.title}</Link></h3>{product.startingPrices[0] ? <strong>{product.startingPrices[0].price} {product.startingPrices[0].currency}</strong> : null}</article>)}</div> : <p className="product-empty">{copy.empty}</p>}</section></main></div>;
}
