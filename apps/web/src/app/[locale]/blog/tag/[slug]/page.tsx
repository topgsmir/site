...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; slug: string }> }): Promise<Metadata> {
  ...
  try {
    const page = await getBlogCollection(locale, { kind: "tags", slug });
    const canonical = `${site}/${locale}/blog/tag/${page.collection.alternateSlugs[locale]}`;
    return {
      title: page.collection.name,
      description: `${page.collection.name} · Top GSM technical journal`,
      alternates: {
        canonical,
        languages: Object.fromEntries(locales.map((code) => [code, `${site}/${code}/blog/tag/${page.collection.alternateSlugs[code]}`]))
      },
...
export default async function TagPage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  const { locale, slug } = await params;
  if (!isLocale(locale)) notFound();
  let page: Awaited<ReturnType<typeof getBlogCollection>> | null = null;
  try {
    page = await getBlogCollection(locale, { kind: "tags", slug });
  } catch (error) {
    if (isApiNotFound(error)) notFound();
  }
