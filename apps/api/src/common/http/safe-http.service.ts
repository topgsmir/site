...
export class SafeHttpService {
  ...
  async request(baseUrl: string, path: string, init: RequestInit, timeoutMs = 12_000) {
    const approved = await this.publicUrls.validate(baseUrl);
    const target = new URL(path, `${approved.url.toString()}/`);
    if (target.origin !== approved.url.origin) throw new BadGatewayException("Provider path escaped its origin");
    ...
    const body = init.body instanceof URLSearchParams ? Buffer.from(init.body.toString(), "utf8") : typeof init.body === "string" ? Buffer.from(init.body, "utf8") : init.body == null ? undefined : (() => { throw new BadGatewayException("Unsupported provider request body"); })();
    if (body && body.length > 1_000_000) throw new BadGatewayException("Provider request is too large");
    const headers = new Headers(init.headers); headers.set("Accept", "application/json"); headers.set("Accept-Encoding", "identity"); if (body) headers.set("Content-Length", String(body.length));
