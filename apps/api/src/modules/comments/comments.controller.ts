...
export class CommentsController {
  ...
  listPublic(@Param("productId", uuid) productId: string, @Query() query: ListCommentsDto) {
    return this.comments.listPublic("product", productId, query);
  }
  ...
  async create(@Param("productId", uuid) productId: string, @Body() body: CreateCommentDto, @Req() request: AuthenticatedRequest, @Ip() ip: string) {
    return this.createFor("product", productId, body, request, ip);
  }

  @Get("blog-posts/:postId")
  listBlogPublic(@Param("postId", uuid) postId: string, @Query() query: ListCommentsDto) {
    return this.comments.listPublic("blog", postId, query);
  }

  @Post("blog-posts/:postId")
  async createBlog(@Param("postId", uuid) postId: string, @Body() body: CreateCommentDto, @Req() request: AuthenticatedRequest, @Ip() ip: string) {
    return this.createFor("blog", postId, body, request, ip);
  }

  private async createFor(target: "product" | "blog", targetId: string, body: CreateCommentDto, request: AuthenticatedRequest, ip: string) {
    ...
    return this.comments.create(target, targetId, body, user);
  }
  ...
  @Get("seller")
  @UseGuards(AuthenticatedGuard)
  listSeller(@Req() request: AuthenticatedRequest, @Query() query: SellerListCommentsDto) { return this.comments.listSeller(request.authenticatedUser!, query); }
...
export class AdminCommentsController {
  ...
  @Post(":id/reply")
  async reply(@Req() request: AuthenticatedRequest, @Ip() ip: string, @Param("id", uuid) id: string, @Body() body: ReplyCommentDto) { await this.limits.consumeCommentReply(request.authenticatedUser!.id, ip); return this.comments.replyEditorial(request.authenticatedUser!, id, body.body); }

  @Post(":id/flag")
  async flag(@Req() request: AuthenticatedRequest, @Ip() ip: string, @Param("id", uuid) id: string) { await this.limits.consumeCommentReply(request.authenticatedUser!.id, ip); return this.comments.flagEditorial(request.authenticatedUser!, id); }
}
